...
class _ClassSelectionScreenState extends State<ClassSelectionScreen> {
  ...
  Future<void> _fetch() async {
    try {
      final res = await http.get(Uri.parse("http://192.168.1.7:8000/get_classes"));
            ...
            return GestureDetector(onTap: () => isEdit ? _dialog(old: n) : Navigator.push(context, MaterialPageRoute(builder: (c) => MainDashboard(selectedClass: n))),
              child: Container(decoration: BoxDecoration(color: const Color(0xFF1E293B), borderRadius: BorderRadius.circular(24), border: Border.all(color: isEdit ? Colors.orangeAccent.withOpacity(0.5) : Colors.white.withOpacity(0.05))),
                child: Stack(children: [
                  ...
                  if (isEdit) Positioned(top: 8, right: 8, child: IconButton(icon: const Icon(Icons.remove_circle_outline, color: Colors.redAccent, size: 20), onPressed: () async { await http.post(Uri.parse("http://192.168.1.7:8000/delete_class"), body: {"name": n}); _fetch(); }))
                ])));
          })),
      ]))),
...
class _TakeAttendanceScreenState extends State<TakeAttendanceScreen> {
  ...
  Future<void> _scan() async {
    ...
    try {
      ...
      var req = http.MultipartRequest('POST', Uri.parse("http://192.168.1.7:8000/scan"));
...
class _StudentListScreenState extends State<StudentListScreen> {
  ...
  Future<void> _fetch() async {
    try {
      final res = await http.get(Uri.parse("http://192.168.1.7:8000/students/${widget.selectedClass}"));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        return Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: const Color(0xFF1E293B), borderRadius: BorderRadius.circular(15), border: Border.all(color: pres ? const Color(0xFF6366F1).withOpacity(0.3) : Colors.transparent)),
          child: ListTile(
            onTap: () async { 
              ...
              else { await http.post(Uri.parse("http://192.168.1.7:8000/toggle_status"), body: {"reg_no": st['reg_no'], "class_name": widget.selectedClass}); _fetch(); } 
            },
      ...
      floatingActionButton: edit ? FloatingActionButton.extended(backgroundColor: Colors.redAccent, onPressed: () async { 
        await http.post(Uri.parse("http://192.168.1.7:8000/bulk_delete"), body: {"reg_nos": sel.join(",")}); 
